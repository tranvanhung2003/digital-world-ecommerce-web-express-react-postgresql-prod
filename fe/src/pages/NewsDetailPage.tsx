    ...
    <>
      {data.news.map((n: any) => (
        <Link
          key={n.id}
          to={`/news/${n.slug}`}
          className="group block bg-neutral-50 dark:bg-neutral-800 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
        >
...
const NewsDetailPage: React.FC = () => {
  ...
  const { data: recentNewsResponse } = useGetNewsQuery({
    limit: 5,
    isPublished: true,
  });
            ...
            {item.tags && (
              <div className="mt-8 flex flex-wrap gap-2">
                {item.tags.split(',').map((tag: string) => (
                  <span
                    key={tag}
                    className="px-3 py-1 bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 text-xs rounded-lg"
                  >
                    #{tag.trim()}
                  </span>
                ))}
            ...
            <div className="mt-16 pt-8 border-t border-neutral-100 dark:border-neutral-800 flex flex-wrap items-center justify-between gap-6">
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  <div className="w-10 h-10 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-400 cursor-pointer hover:bg-primary-500 hover:text-white transition-all">
                    F
                  </div>
                  <div className="w-10 h-10 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-400 cursor-pointer hover:bg-primary-500 hover:text-white transition-all">
                    T
